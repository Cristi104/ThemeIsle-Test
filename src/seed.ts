...
function initializeSchema(db: Database) {
    ...
    CREATE TABLE IF NOT EXISTS clients (
      ...
      phone TEXT,
      address TEXT,
	  -- for user registration and authentification
    ...
    CREATE TABLE IF NOT EXISTS coins (
      ...
      value REAL NOT NULL,
	  -- storing the bitSlow hash in the data base to cut unnecesary performance loss
  ...
  `);
	...
	db.transaction(() => {
		for (let i = 0; i < count; i++) {
			...
			let computedBitSlow = computeBitSlow(bit1, bit2, bit3);
